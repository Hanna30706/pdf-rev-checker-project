#!/usr/bin/env python3
"""
PDF 추출 결과 개선 (하이브리드 방식)
1. docling3_verification.md: pymupdf 중심 본문 + docling 표 구조
2. docling3_process_verification.md: docling 중심 + paddleOCR 이미지
"""
...
def improve_verification_v1(input_file: str, output_file: str, pdf_path: str):
    ...
    원칙:
    - 본문 텍스트/제목/절 번호/문단순서/페이지는 PyMuPDF 기준
    - 표가 아닌 영역은 docling 덮어쓰지 않음
    - 표 영역에서만 docling 표 구조 활용
    - 표 셀은 pymupdf 좌표 기반 텍스트
    """
    print(f"[1번] 개선 시작: {input_file}")

    with open(input_file, 'r', encoding='utf-8') as f:
        content = f.read()

    # 섹션 분리
    docling_match = re.search(r'(?:^|\n)## Docling 추출 결과|(?:^|\n)\*\*Docling 추출 결과\*\*',
                              content, re.MULTILINE)
    pymupdf_match = re.search(r'(?:^|\n)## PyMuPDF 페이지별 텍스트|(?:^|\n)\*\*PyMuPDF 페이지별 텍스트\*\*',
                              content, re.MULTILINE)
    ...
    print(f"  [INFO] PyMuPDF 페이지: {len(pymupdf_pages)}개")

    # Docling에서 표 추출
    tables = extract_tables_from_docling(docling_section)
    print(f"  [INFO] Docling 표: {len(tables)}개")

    # 개선된 출력 작성
    improved = f"""# [I-AG1-0401-G01]언론홍보 및 대응 지침_r0
...
def improve_verification_v2(input_file: str, output_file: str, pdf_path: str):
    """
    2번 PDF 개선: Docling 중심 (이미 우수) + PaddleOCR (선택)

    원칙:
    - Docling을 기본 결과로 사용
    - 프로세스 맵 이미지만 PaddleOCR로 추출
    """
    print(f"[2번] 개선 시작: {input_file}")
    ...
    try:
        ...
        print("  - PaddleOCR으로 이미지 텍스트 추출 중...")
        ...
        for page_num, page in enumerate(doc, 1):
            ...
            for img_index, xref in enumerate(images):
                try:
                    ...
                    img_file = f"/tmp/ocr_{page_num}_{img_index}.png"
                    pix.save(img_file)

                    result = ocr.ocr(img_file, cls=True)

                    if result:
                        ocr_section += f"### 페이지 {page_num}, 이미지 {img_index + 1}\n\n"
        ...
        doc.close()
        print(f"  - {extracted_count}개 이미지에서 텍스트 추출")

    except ImportError:
        ocr_section += "PaddleOCR이 설치되지 않았습니다.\n"
        ocr_section += "설치: pip install paddleocr paddlepaddle\n"
        print("  ⚠ PaddleOCR 미설치 (선택사항)")

    except Exception as e:
        ocr_section += f"OCR 오류: {str(e)}\n"
        print(f"  ⚠ OCR 오류: {e}")

    # 결과 작성
    improved = f"""{content}{ocr_section}"""
...
def main():
    ...
    # 1번: 언론홍보 및 대응 지침
    pdf1 = 'pdf-compare-v2/[I-AG1-0401-G01]언론홍보 및 대응 지침_r0.pdf'
    if Path(pdf1).exists():
        ...
        print(f"[ERROR] PDF 파일 없음: {pdf1}")

    print()

    # 2번: 국내 업면허 관리
    pdf2 = 'pdf-compare-v2/[P-MGG-0601-010]국내 업면허 관리_r0.pdf'
    if Path(pdf2).exists():
