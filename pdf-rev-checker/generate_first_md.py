...
**페이지 크기:** {page['width']} x {page['height']} pt
...
"""
                ...
                md += f"""- **크기:** {len(rows)} 행 x {cols} 열 = {total_table_cells}개 셀
...
if __name__ == "__main__":
    pdf_path = Path(__file__).parent / "[I-AGG-0301-030-A01]부표1. 현장점검 수행 및 평가기준_개정 전.pdf"
    ...
    try:
        ...
        # first.md로 저장
        output_path = Path(__file__).parent / "first.md"
        ...
        print(f"   - Lines: {len(md_content.split(chr(10)))}")
        ...
        for page in data["content"]:
            ...
            print(f"   - PAGE {page['page_num']}: Headings {headings}, Paragraphs {paragraphs}, Tables {tables}, Lists {lists}")
